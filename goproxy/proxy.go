...
// 上游分发代理
type ForwardBeforeFunc func(r *http.Request)
type ForwardAfterFunc func(r *http.Response)
type ForwardUpstream struct {
	Address string
	Before  ForwardBeforeFunc
	After   ForwardAfterFunc
}
...
func WithForward(upstream []ForwardUpstream) Option {
	return func(opt *options) {
...
func (p *Proxy) ForwardToUpstream(ctx *Context, upStream ForwardUpstream, body []byte) *http.Response {
	...
	newRequest.Header = CloneHeader(originReq.Header)
	newRequest.Host = originReq.Host
	newRequest.URL.Host = originReq.URL.Host
	newRequest.URL.Path = originReq.URL.Path

	if upStream.Before != nil {
		upStream.Before(newRequest) // 预处理
	}

	// 设置代理
	proxy, err := url.Parse(upStream.Address)
	...
	resp, err = cli.Do(newRequest)
	if err != nil {
		log.Print("Error: ", err)
		return nil
	}

	if upStream.After != nil {
		upStream.After(resp)
	}
...
func (p *Proxy) DoRequest(ctx *Context, responseFunc func(*http.Response, error)) {
	log.Printf("DoRequest => [%s]|%s", ctx.Req.Method, ctx.Req.URL.String())
	if ctx.Data == nil {
